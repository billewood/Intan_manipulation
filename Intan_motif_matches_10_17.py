...
sound_length_thresh = 5000 # excludes sound periods shorter than this (in data points for now) when searching for templates
mic_lowpass = 700
mic_highpass = 11000
...
# #            freq_corr[freq_i] = correlate(in1,in2, mode = 'valid') #alternate way, not sure of the specifics
...
def plot_template_specs(temp_t, templ_freq, templ_timefreq):
...
def get_symmetric_spect_corr(sound_timefreq, templ_timefreq, templ_len, fs_mic = 25000, spec_sample_rate = 1000, freq_spacing = 50, zscore = 0):
    ...
    for t in range((templ_timefreq.shape[1]-1) * 2): #
        ...
        if t < templ_timefreq.shape[1]: # sliding time, first half of timepoints
            in1 = templ_timefreq.T[:][-t:] # take all frequencies, restricted time- transposing seems like a weird way to do thsi but it's all ive got
            in2 = sound_timefreq.T[:][0:t]
        else: # sliding time, second half of timepoints
            in1 = templ_timefreq.T[:][0:templ_timefreq.shape[1] * 2 - t]            
            in2 = sound_timefreq.T[:][-(templ_timefreq.shape[1] - t):]
        # correlate relative freq band to each other
...
#==============================================================================
#             if t < templ_timefreq.shape[1]:
#                 in1 = templ_timefreq[freq_i][-t:]            
#                 in2 = sound_timefreq[freq_i][0:t]
#             else:
#                 in1 = templ_timefreq[freq_i][0:templ_timefreq.shape[1] * 2 - t]            
#                 in2 = sound_timefreq[freq_i][-(templ_timefreq.shape[1] - t):]
#==============================================================================
...
# 
# f_corr = list()
# for freq in range(template_timefreq.shape[0]):
#     f_corr.append(correlate(np.abs(template_timefreq[freq]), np.abs(sound_timefreq[freq])))
#     sum_f_corr = np.sum(sum_f_corr,f_corr)
# sum_f_corr = np.zeros(len(f_corr[0]))
# for freq in range(len(f_corr)):
#     sum_f_corr = sum_f_corr + f_corr[freq]
#==============================================================================
...
for i in range(len(template_freq[0])):
    if (template_freq[0][i] > low_freq_corr) & (template_freq[0][i] < high_freq_corr): # change this to np.where FUTURE
        freq_index.append(i)
        
# TODO MAkes this a function
# TODO Decide if it's worth keeping this as I don't really use it anymore- it does speed things up
...
for template in range(num_templates):   # the number of templates, 
    ...
    for i in range(len(sound_onset)):
        ...
        sound_timefreq = zscore(np.abs(sound_timefreq), axis = None) 
        templ_t, template_freq, templ_timefreq, rms = spectrogram(template_wav[template], fs_mic, spec_sample_rate, freq_spacing)
        templ_timefreq = templ_timefreq[freq_index,:]
        templ_len = len(templ_t)
        templ_timefreq = zscore(np.abs(templ_timefreq), axis = None) 
 #       spect_corr = get_symmetric_spect_corr(sound_wav, template_wav[template], freq_index, fs_mic)
        spect_corr, amplitude_corr = get_symmetric_spect_corr(sound_timefreq, templ_timefreq, templ_len, fs_mic)
...
#==============================================================================
# THIS IS THE END OF THE WORKING CODE< BLEOW IS USER DEFINED STUFF
# =============================================================================
    
# draw the spectrogram of the templates to remind yourself what they look like, if you want
plot_template_specs(templ_t, template_freq, template_timefreq)
# =============================================================================
...
plot(max_corrs[0] ,max_corrs[2], max_corrs[3],'or')
plot(np.squeeze(max_corrs[0])[nonplayback],np.squeeze(max_corrs[2])[nonplayback], np.squeeze(max_corrs[3])[nonplayback],'o')
