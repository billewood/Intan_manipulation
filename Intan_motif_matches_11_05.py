...
experiment_file = os.path.join(experiment_dir, "RHD", "LbY6074__161216_142224.rhd")
stimulus_file = os.path.join(experiment_dir, "pyoperant", "LbY6074_trialdata_20161216142333.csv") # set to 0 if it doesn't exist
...
stims_given = 1 # if there were auditory stimuli given or not
avoid_stims = 1 # 1 means avoid the vocal periods with stims, 0 means there are stims but don't avoid them, irrelevant
# if stims_give == 0 , MAY BE CHANGED IN CODE
    ...
    while keep_looking and i < len(sound_onset-2):
        ...
        if stims_present == 1:
            if avoid_stims and sound_playback[i] > 0:
                ...
                continue # TODO NEED TO CHECK THIS CONTINUE PASSES BACK CORRECTLY, NOT SURE IT DOES
...
if stims_given == 1:
    ...
    soundplayback = np.zeros(len(sound_onset))
    nonplayback = soundplayback + 1
    stim_env = ()
    stimuli = ()
    
#### beginning user controlled template interfaces   
spect_figure = figure('spectrogram') #this figure will be used to define the template
coords = []
cid = int
...
if (sound_offset - sound_onset).any > too_long: # if we have sound periods longer than ~30 s it hangs on them for hours per template.
    print("Warning, some sound periods too long ( > %s s.), not analyzing those sounds" %(too_long/fs_mic))
...
# with open('BlYe0923__160829_144329_11_02') as f:  # Python 3: open(..., 'rb')
...
# with open('BlYe0923__160829_144329_11_02', 'w') as f:  
#     pickle.dump([father_song_index, sound_onset, sound_offset, template_correlations, amplitude_correlations, amp_ind, corr_ind, template_t, template_freq, template_timefreq, nonplayback, max_corrs, max_amp_corrs], f)
# 
#==============================================================================
#==============================================================================
...
alignments = plot_high_corrs(high_corrs, template, mic, template_correlations, amplitude_correlations, corr_ind, amp_ind) # simply returns alignment times
