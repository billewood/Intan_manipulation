...
too_long = 10 # excludes sound onsets longer than this since they will make the program take forever
...
vd_stds = 2 # num of stds for vocal density threshold
vd_win = .1 # in s
...
def plot_stim_and_mic(onsets_to_plot,sound_playback, stimuli, sound_onset, sound_offset, close_fig = 1, pause_time = .1):
    ...
    for onset in onsets_to_plot:
        ...
        if close_fig:
            close(sp_stim_fig) 
    return
def user_select_templates(mic, sound_onset, sound_offset, fs_mic, sound_length_thresh = 0):
    """ 
    ...
    while keep_looking and i < len(sound_onset-2):
        i += 1
        if sound_offset[i] - sound_onset[i] < sound_length_thresh * fs_mic: # only implementing for long templates at the moment
...
def plot_high_corrs(sound_onset, high_corrs, x, y, mic, fs_mic, template_correlations, amplitude_correlations, corr_ind, amp_ind, plot_figures = 0):
    """
    ...
    figure(1)
    title('template')
    plot_zscore_spectrogram(template_t[x], template_freq[x], template_timefreq[x])   
#    plot_zscore_spectrogram(template_t[y], template_freq[y], template_timefreq[y])   
    ...
    for sound in high_corrs:
        ...
        good_align = np.int(sound_onset[sounds[sound]]) + align - np.int(np.round(.5 * len(template_correlations[x][sound]))) 
        amp_good_align = np.int(sound_onset[sounds[sound]]) + a_align - np.int(np.round(.5 * len(amplitude_correlations[x][sound])))         
        sound_align_wav = mic[good_align - fs_mic : good_align + 2 * fs_mic]
        amp_align_wav = mic[amp_good_align - fs_mic : amp_good_align + 2 * fs_mic]
        if plot_figures:        
            sound_t, sound_freq, sound_timefreq, sound_rms = spectrogram(np.squeeze(sound_align_wav), fs_mic, spec_sample_rate = 1000, freq_spacing = 50)
...
if normalize_mic:
    ...
    previous_i = 0
    while i < len(mic):
        i = i + np.round(fs_mic / 2) # filter by the half second, not optimized
        if i > len(mic):
            i = len(mic)
...
# if no playback set to all    
nonplayback = range(len(sound_onset)) #, dtype=np.int) + int(1)# index of sound playbacks
...
coords = []
cid = int
template_wav, template_index, template_t, template_freq, template_timefreq, template_starts, template_ends = user_select_templates(mic, sound_onset, sound_offset, fs_mic, sound_length_thresh)

spect_figure = figure('spectogram') # after finding some calls sometimes it's nice to turn up the thresholds to search just for good songs
coords = []
cid = int
...
if (sound_offset - sound_onset).any > too_long:
    print("Warning, some sound periods too long ( > %s), not analyzing those sounds" %(too_long/fs_mic))
...
with open('LBlYe0923__160830_120325', 'w') as f:  
    pickle.dump([sound_onset, sound_offset, sounds, template_correlations, amplitude_correlations, amp_ind, corr_ind, template_t, template_freq, template_timefreq, nonplayback, max_corrs, max_amp_corrs], f)

# Getting back the objects:
with open('LBlYe0923__160830_120325') as f:  # Python 3: open(..., 'rb')
    sound_onset, sound_offset, sounds, template_correlations, amplitude_correlations, amp_ind, corr_ind, template_t, template_freq, template_timefreq, nonplayback, max_corrs, max_amp_corrs = pickle.load(f)

with open('BlYe0923__160830_120325_alignments') as f:  
    tutee_songs, tutee_alignments, father_songs, father_alignments = pickle.load(f)

# For pickling important variables (custom)
with open('BlYe0923__160830_120325_alignments', 'w') as f:  
    pickle.dump([tutee_songs, tutee_alignments, father_songs, father_alignments], f)


with open('BlYe0923__160829_144329_11_02', 'w') as f:  
    pickle.dump([father_song_index, sound_onset, sounds, sound_offset, template_correlations, amplitude_correlations, amp_ind, corr_ind, template_t, template_freq, template_timefreq, nonplayback, max_corrs, max_amp_corrs], f)

# getting back custom vars
with open('BlYe0923__160830_120325_alignments') as f:  
    tutee_songs, tutee_alignments, father_songs, father_alignments = pickle.load(f)


#==============================================================================
# THIS IS THE END OF THE WORKING CODE< BLEOW ARE THE MOST GENERIC PLOTS
# =============================================================================
...
y = 2
figure
plot(max_corrs[x], max_corrs[y],'or')    
plot(np.squeeze(max_corrs[x])[nonplayback],np.squeeze(max_corrs[y])[nonplayback],'ob')
# or a 3d plot
...
y = 1
plot_figures = 1
corr_thresh = [11000, 11000]
high_corrs = np.squeeze(np.where( (np.asarray(max_corrs[x] > corr_thresh[0])) and (np.asarray(max_corrs[y]) > corr_thresh[1])))
alignments, amp_alignments = plot_high_corrs(sound_onset, high_corrs, x, y, mic, fs_mic, template_correlations, amplitude_correlations, corr_ind, amp_ind, plot_figures) # simply returns alignment times
high_nonplayback = common_elements(high_corrs, nonplayback) # high correlations that aren't playbacks
#ampcorr

# for instance save like this:
tutee_song_ind = sounds[high_corrs]
tutee_aligned_times = alignments
tutee_amp_aligned_times = amp_alignments
# or
father_song_ind = sounds[high_corrs]
father_aligned_times = alignments
father_amp_aligned_times = amp_alignments

with open('BlYe0923__160830_120325_alignments', 'w') as f:  
    pickle.dump([tutee_song_ind, tutee_aligned_times, tutee_amp_aligned_times, father_song_ind, father_aligned_times, father_amp_aligned_times], f)


plot_figures = 1
corr_thresh = [700000, 700000]
high_corrs = np.squeeze(np.where( (np.asarray(max_amp_corrs[x] > corr_thresh[0])) and (np.asarray(max_amp_corrs[y]) > corr_thresh[1])))
alignments, amp_alignments = plot_high_corrs(sound_onset, high_corrs, x, y, mic, fs_mic, template_correlations, amplitude_correlations, corr_ind, amp_ind, plot_figures) # simply returns alignment times
high_nonplayback = common_elements(high_corrs, nonplayback) # high correlations that aren't playbacks
...
realignments = realign(sound_onset, tutee_songs, x, high_corrs, template_correlations, amplitude_correlations, corr_ind, amp_ind)



#this plots correlations vs each other, nonplaybacks in blue
x = 0
y = 3
z = 4
fig = figure() ## spect_corrs
ax = fig.add_subplot(111, projection = '3d')
plot(np.squeeze(max_corrs[x]), np.squeeze(max_corrs[y]), np.squeeze(max_corrs[z]),'or')
plot(np.squeeze(max_corrs[x])[nonplayback],np.squeeze(max_corrs[y])[nonplayback], np.squeeze(max_corrs[z])[nonplayback],'o')
title('spectral correlations, blue nonplayback, red has some playback')
xlabel('x-axis')
ylabel('y-axis')

# this plots correlations that are high (specific for a certain syllable, perhaps song), with nonplaybacks (actual vocalizations) in blue
x = 0
y = 3
z = 4
fig = figure() ## spect_corrs
...
plot(np.squeeze(max_corrs[x])[high_nonplayback], np.squeeze(max_corrs[y])[high_nonplayback], np.squeeze(max_corrs[z])[high_nonplayback],'ob')
