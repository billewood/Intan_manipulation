...
p_file_dir = '/auto/tdrive/billewood/intan data/LBlYel0923/pickle data'
...
with open(p_file) as f:  
    tutee_song_ind, tutee_aligned_times, tutee_amp_aligned_times, father_song_ind, father_aligned_times, father_amp_aligned_times = pickle.load(f)

syllables = father_aligned_times
...
fig_clean = figure(figsize=(15,10))  
suptitle('Clean Figure')
fig_suppressed = figure(figsize=(15,10))  
suptitle('Suppressed Figure')
fig_raw = figure(figsize=(15,10))
suptitle('Raw Figure')  
fig_all_elect_raw = figure(figsize=(15,10))
suptitle('Raw Electrodes')
fig_all_elect_clean = figure(figsize=(15,10))
suptitle('Clean Electrodes')
fig_all_elect_sup = figure(figsize=(15,10))
suptitle('all elect sup')

gs = gridspec.GridSpec(30 * (len(syllables) + 1), 1) #I added teh five to shorten for troubleshooting
for i in range(syllablesT.shape[0]):   # shortened for troubleshooting
    ...
    if i == 0:
        ...
        plot_spectrogram(to, fo, spect, ax=ax, 
                ticks=False, fmin=250, fmax=8000, colormap=None, colorbar=False, log = True, dBNoise = 50)
        ax = fig_raw.add_subplot(gs[0:29,0])
        to, fo, spect, rms = spectrogram(mic_slice, sample_rate, 1000, 50)                  
        plot_spectrogram(to, fo, spect, ax=ax, 
                ticks=False, fmin=250, fmax=8000, colormap=None, colorbar=False, log = True, dBNoise = 50)
        ax = fig_suppressed.add_subplot(gs[0:29,0])
        to, fo, spect, rms = spectrogram(mic_slice, sample_rate, 1000, 50)                  
        plot_spectrogram(to, fo, spect, ax=ax, 
                ticks=False, fmin=250, fmax=8000, colormap=None, colorbar=False, log = True, dBNoise = 50)
        ax = fig_all_elect_raw.add_subplot(gs[0:29,0])
        to, fo, spect, rms = spectrogram(mic_slice, sample_rate, 1000, 50)                  
        plot_spectrogram(to, fo, spect, ax=ax, 
                ticks=False, fmin=250, fmax=8000, colormap=None, colorbar=False, log = True, dBNoise = 50)  
        ax = fig_all_elect_clean.add_subplot(gs[0:29,0])
        to, fo, spect, rms = spectrogram(mic_slice, sample_rate, 1000, 50)                  
        plot_spectrogram(to, fo, spect, ax=ax, 
                ticks=False, fmin=250, fmax=8000, colormap=None, colorbar=False, log = True, dBNoise = 50) 
        ax = fig_all_elect_sup.add_subplot(gs[0:29,0])
        to, fo, spect, rms = spectrogram(mic_slice, sample_rate, 1000, 50)                  
        plot_spectrogram(to, fo, spect, ax=ax, 
                ticks=False, fmin=250, fmax=8000, colormap=None, colorbar=False, log = True, dBNoise = 50) 
    ...
    for ich1 in range(16):
        ...
        high_amp_clean[:,ich1] *= gain # from 0 to 1, where 0 is high
        cut = np.where(gain < (np.mean(gain) - (4 * np.std(gain))))
        high_amp_suppressed[:,ich1] = high_amp_clean[:,ich1]
        high_amp_suppressed[cut, ich1] = 0
    ...
    t_end = np.asarray(t_amp[-1] - t_amp[0])
    ax_raw = fig_raw.add_subplot(gs[30*(i+1):30*(i+1)+29,0])
    ax_raw.plot(t_amp-t_amp[0], high_amp[:, ielec] / np.std(high_amp[:, ielec]), linewidth = 0.5, color = 'black')
    ax_raw.plot(t_mic-t_mic[0], mic_env/max_mic, linewidth = 1, color= 'red')
    ax_raw.tick_params(labelbottom='off', labelleft='off')
    ax_raw.set_ylim ([-8, 8])
    ax_raw.set_xlim([0, t_end])


    ax_sup = fig_suppressed.add_subplot(gs[30*(i+1):30*(i+1)+29,0])
    ax_sup.plot(t_amp-t_amp[0], high_amp_suppressed[:, ielec] / np.std(high_amp_suppressed[:, ielec]), linewidth = 0.5, color = 'black')
    ax_sup.plot(t_mic-t_mic[0], mic_env/max_mic, linewidth = 1, color= 'red')
    ax_sup.set_ylim ([-8, 8])
    ax_sup.tick_params(labelbottom='off', labelleft='off')
    ax_sup.set_xlim([0, t_end])


    ax_clean = fig_clean.add_subplot(gs[30*(i+1):30*(i+1)+29,0])
    ax_clean.plot(t_amp-t_amp[0], high_amp_clean[:, ielec] / np.std(high_amp_clean[:, ielec]), linewidth = 0.5, color = 'black')
    ax_clean.plot(t_mic-t_mic[0], mic_env/max_mic, linewidth = 1, color= 'red')    
    ax_clean.tick_params(labelbottom='off', labelleft='off')
    ax_clean.set_ylim ([-8, 8])
    ax_clean.set_xlim([0, t_end])
    
    if i == 0:
        for elect in range(16):
            ax_all_elect_clean = fig_all_elect_clean.add_subplot(gs[30*(elect+1):30*(elect+1)+29,0])
            ax_all_elect_clean.plot(t_amp-t_amp[0], high_amp_clean[:, elect] / np.std(high_amp_clean[:, elect]), linewidth = 0.5, color = 'black')
            ax_all_elect_clean.plot(t_mic-t_mic[0], mic_env/max_mic, linewidth = 1, color= 'red')    
            ax_all_elect_clean.tick_params(labelbottom='off', labelleft='off')
            ax_all_elect_clean.set_ylim ([-8, 8])
            ax_all_elect_clean.set_xlim([0, t_end])
        
            ax_all_elect_raw = fig_all_elect_raw.add_subplot(gs[30*(elect+1):30*(elect+1)+29,0])
            ax_all_elect_raw.plot(t_amp-t_amp[0], high_amp[:, elect] / np.std(high_amp[:, elect]), linewidth = 0.5, color = 'black')
            ax_all_elect_raw.plot(t_mic-t_mic[0], mic_env/max_mic, linewidth = 1, color= 'red')    
            ax_all_elect_raw.tick_params(labelbottom='off', labelleft='off')
            ax_all_elect_raw.set_ylim ([-8, 8])
            ax_all_elect_raw.set_xlim([0, t_end])
            
            ax_all_elect_sup = fig_all_elect_sup.add_subplot(gs[30*(elect+1):30*(elect+1)+29,0])
            ax_all_elect_sup.plot(t_amp-t_amp[0], high_amp_suppressed[:, elect] / np.std(high_amp_suppressed[:, elect]), linewidth = 0.5, color = 'black')
            ax_all_elect_sup.plot(t_mic-t_mic[0], mic_env/max_mic, linewidth = 1, color= 'red')    
            ax_all_elect_sup.tick_params(labelbottom='off', labelleft='off')
            ax_all_elect_sup.set_ylim ([-8, 8])
            ax_all_elect_sup.set_xlim([0, t_end])
